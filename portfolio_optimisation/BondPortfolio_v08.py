...
model += lpSum(x[j] * sov_mv[j] for j in range(200)) >= 0.075 * lpSum(x[j] * bonds_mv[j] for j in range(200))
model += lpSum(x[j] * sov_mv[j] for j in range(200)) <= 0.33 * lpSum(x[j] * bonds_mv[j] for j in range(200))
model += lpSum(x[j] * corp_mv[j] for j in range(200)) <= 0.5 * lpSum(x[j] * bonds_mv[j] for j in range(200))
model += lpSum(x[j] * illiquid_mv[j] for j in range(200)) >= 0.1 * lpSum(x[j] * bonds_mv[j] for j in range(200))
model += lpSum(x[j] * illiquid_mv[j] for j in range(200)) <= 0.27 * lpSum(x[j] * bonds_mv[j] for j in range(200))
model += lpSum(x[j] * bbb_mv[j] for j in range(200)) <= 0.25 * lpSum(x[j] * bonds_mv[j] for j in range(200))
model += lpSum(x[j] * below_bbb_mv[j] for j in range(200)) <= 0.33 * lpSum(x[j] * bonds_mv[j] for j in range(200))
